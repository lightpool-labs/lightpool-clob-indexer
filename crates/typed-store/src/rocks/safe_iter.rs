...
/// Iterate keys with typed deserialization, but keep values as raw bytes.
/// Unlike [`SafeIter`], failed value BCS decoding cannot hide rows.
pub struct SafeRawValueIter<'a, K> {
    cf_name: String,
    db_iter: RawIter<'a>,
...
impl<'a, K: DeserializeOwned> SafeRawValueIter<'a, K> {
    pub(super) fn new(
        cf_name: String,
        db_iter: RawIter<'a>,
        _timer: Option<HistogramTimer>,
...
impl<K: DeserializeOwned> Iterator for SafeRawValueIter<'_, K> {
    type Item = Result<(K, Vec<u8>), TypedStoreError>;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.db_iter.valid() {
            ...
            let value = raw_value.to_vec();
            self.db_iter.next();
            match key {
                Ok(k) => Some(Ok((k, value))),
                Err(err) => Some(Err(TypedStoreError::SerializationError(format!("{err}")))),
            }
        } else {
            match self.db_iter.status() {
                Ok(_) => None,
...
impl<K> Drop for SafeRawValueIter<'_, K> {
    fn drop(&mut self) {
        if let Some(bytes_scanned) = self.bytes_scanned.take() {
            bytes_scanned.observe(self.bytes_scanned_counter as f64);
        }
