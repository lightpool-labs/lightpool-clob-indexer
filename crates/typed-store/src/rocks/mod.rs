...
use crate::rocks::safe_iter::{SafeIter, SafeRawValueIter, SafeRevIter};
...
impl<K, V> DBMap<K, V> {
    ...
    /// Iterate typed keys with raw value bytes (no BCS decode of `V`).
    pub fn safe_iter_raw_values(&self) -> SafeRawValueIter<'_, K>
    where
        K: Serialize + DeserializeOwned,
    {
    ...
    /// Count RocksDB rows without deserializing values.
    pub fn raw_entry_count(&self) -> Result<usize, TypedStoreError> {
        let mut count = 0usize;
        let mut db_iter = self.db.raw_iterator_cf(&self.cf, self.opts.readopts());
        ...
        db_iter
            .status()
            .map_err(|err| TypedStoreError::RocksDBError(format!("{err}")))?;
        Ok(count)
    }
